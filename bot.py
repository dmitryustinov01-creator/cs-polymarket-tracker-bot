...
results = [m for m in markets if is_cs_market(m)]
...
```
if len(teams) >= 2:
    return f"{format_team_with_rank(teams[0], p0)}  vs  {format_team_with_rank(teams[1], p1)}"
elif len(teams) == 1:
    ...
    while True:
        try:
            ...
            if is_first_run:
                ...
                log.info(f"[{now}] Check done. Known: {len(known_market_ids)}, New: {len(new_markets)}")

            if new_markets and subscribers:
                for market in new_markets:
                    text = build_new_market_message(market)
                    kb = InlineKeyboardMarkup(inline_keyboard=[[
                        InlineKeyboardButton(text="📈 Открыть рынок", url=market_url(market))
                    ]])
